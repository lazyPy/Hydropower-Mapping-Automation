...
class DataProcessor:
    ...
    def load_and_validate_dem(self) -> Tuple[Dataset, RasterLayer]:
        ...
        try:
            ...
            with rasterio.open(dem_to_use) as src:
                ...
                # Create RasterLayer metadata
                stats = src.statistics(1)  # Band index 1
                ...
                self.console.info(f"Elevation range: {stats.min:.1f}m - {stats.max:.1f}m")
    ...
    def load_discharge_data(self) -> bool:
        """Step 5: Load discharge/rainfall data (optional)"""
        self.console.step("Loading Discharge Data")
        ...
        if not discharge_files:
            self.console.warning("No discharge data found, will use estimated values")
    ...
    def process_all(self) -> bool:
        ...
        try:
            ...
            # Step 6: Load discharge data (optional)
            self.load_discharge_data()
            
            # Step 7: Run site pairing
